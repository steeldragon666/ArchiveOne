...
| `cloud_sync_synced_file`   | Internal de-duplication ledger for the cloud-sync polling job (Google Drive et al., 0075); not directly addressable by end users                       | Server-side privileged path only (polling job); transitive tenant scope via `connection_id` FK to `cloud_sync_connection` (RLS-enforced); `cpa_app` has `SELECT, INSERT` only — `UPDATE`/`DELETE` revoked |
...
| P9.3 federation | 2026-05-07 | Aaron    | Migration 0070 adds three new tables (federation_share, federation_invitation, federation_audit) — all RLS-protected. Migration 0071 extends existing claim/activity/expenditure/narrative_draft policies with cross-tenant read clause via federation_share.                                        |
| Drift fix       | 2026-05-22 | Aaron    | Migration `0075_cloud_sync_connection.sql` shipped `cloud_sync_synced_file` (intentionally non-RLS — internal de-duplication ledger for the polling job) without updating the exempt list; CI audit caught it. Added entry with rationale and compensating controls.                                 |
