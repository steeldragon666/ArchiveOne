 ...
 *
 * The SQL stub mirrors the postgres-js template-tag interface (see
 * `payroll/xero-payroll/employee-sync.test.ts` for the same pattern).
 * Each call captures the joined SQL string + parameter list, plus an
 * optional row-set the test pre-loads to simulate SELECT/RETURNING
 * results.
 */
...
const FIXTURE_PATH = resolve(
  here,
  '../../../../tests/fixtures/xero-accounting/invoices-sample.json',
);
const FIXTURE = JSON.parse(readFileSync(FIXTURE_PATH, 'utf8')) as {
  Invoices: Array<{ InvoiceID: string; Type: string; LineItems: unknown[] }>;
};

// Sanity: the fixture has the shape the suite assumes.
const FIXTURE_ACCPAY = FIXTURE.Invoices.filter((i) => i.Type === 'ACCPAY');
const FIXTURE_ACCREC = FIXTURE.Invoices.filter((i) => i.Type === 'ACCREC');
...
interface SqlStub {
  ...
  /** Queue a SELECT-existing-expenditure row-set (FIFO across invoices). */
...
const conn = (): SyncInvoicesConnection => ({
  id: CONNECTION_ID,
  tenant_id: TENANT_ID,
...
/**
 * Pre-load row-sets for an INSERT-path invoice. The sync function:
 ...
 */
function queueNewInvoiceRows(stub: SqlStub, expenditureId: string): void {
...
function queueExistingInvoiceRows(stub: SqlStub, expenditureId: string): void {
...
// -------------------------------------------------------------------------
// 1. Backfill mode: paginated fetch (mock 2 pages), all ACCPAY rows mapped.
// -------------------------------------------------------------------------

test('backfill: paginates until short page; all ACCPAY rows persisted', async () => {
  // Page 1: 100 ACCPAY rows → triggers page=2 follow-up.
  const page1Invoices = Array.from({ length: 100 }, (_, i) => ({
    InvoiceID: `00000000-0000-4000-8000-${String(i).padStart(12, '0')}`,
    Type: 'ACCPAY',
    Status: 'AUTHORISED',
  ...
  const page2Invoices = FIXTURE_ACCPAY;

  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, { Invoices: page1Invoices });

  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '2', pageSize: '100' })
    .reply(200, { Invoices: page2Invoices });
  ...
  // page1: 100 invoices × 1 line. page2: AWS 2 + Officeworks 2 + Telstra 2 = 6 lines.
  assert.equal(result.lines, 100 + 6);
...
test('incremental: sets If-Modified-Since header and forwards `since` correctly', async () => {
  ...
  nock(XERO_API_HOST)
    ...
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, { Invoices: FIXTURE_ACCPAY });
...
// -------------------------------------------------------------------------
// 3. ACCREC rows filtered out — neither persisted nor counted.
// -------------------------------------------------------------------------

test('ACCREC rows are filtered out (defensive guard) — only ACCPAY persisted', async () => {
  // Even if the API filter ever drops, the local `inv.Type !== 'ACCPAY'`
  // guard keeps sales invoices out of expenditure. This test sends the
  // FULL fixture (incl. the ACCREC row) and asserts only 3 are persisted.
  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, { Invoices: FIXTURE.Invoices });
  ...
  assert.equal(result.fetched, 3, 'ACCREC must not count toward fetched');
  assert.equal(result.inserted, 3);
  assert.equal(result.events_written, 3);
  // No EXPENDITURE_INGESTED for the sales invoice.
  assert.ok(
    chainStub.calls.every((c) => {
      const p = c.payload as { vendor_name: string };
      return p.vendor_name !== 'Customer Pty Ltd';
    }),
    'sales-invoice contact must not appear in any event payload',
  );
});
...
test('idempotency: existing rows UPDATE without writing EXPENDITURE_INGESTED', async () => {
  ...
  const sqlStub = makeSqlStub();
  // Pre-queue: each invoice's SELECT returns an existing row (UPDATE path).
  for (let i = 0; i < 3; i++) {
    queueExistingInvoiceRows(sqlStub, `existing-exp-${i}`);
  }
  const chainStub = makeChainStub();
  ...
  // Expect: 3 SELECTs + 3 UPDATEs + (3 DELETE + 6 INSERT lines) = 15 queries.
  // Lines: AWS 2 + Officeworks 2 + Telstra 2 = 6 line INSERTs.
...
// -------------------------------------------------------------------------
// 5. Non-AUD invoice throws a descriptive error.
// -------------------------------------------------------------------------

test('non-AUD invoice throws descriptive error before INSERT', async () => {
  const usdInvoice = {
    InvoiceID: 'usd-invoice-1',
    Type: 'ACCPAY',
    Status: 'AUTHORISED',
    Date: '/Date(1640995200000+0000)/',
  ...
  await assert.rejects(
    ...
    /Non-AUD invoice unsupported in P4: tenant=.* invoice=usd-invoice-1 currency=USD/,
  );
});
...
test('Invoices field omitted entirely → treated as empty page', async () => {
  // Defensive: Xero may return `{}` rather than `{ Invoices: [] }` for a
  // 304 / no-content branch (the response we treat as "no changes").
  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, {});
...
test('EXPENDITURE_INGESTED payload matches ExpenditureIngestedPayload', async () => {
  // Only one fixture invoice — easier to assert on the exact payload.
  const inv = FIXTURE_ACCPAY[0]!;
  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
...
// -------------------------------------------------------------------------
// 8. ACCPAY filter applied via the `where` query parameter.
// -------------------------------------------------------------------------

test('uses Xero `where` parameter to filter ACCPAY at the API layer', async () => {
  ...
  assert.equal(capturedQuery.get('where'), 'Type=="ACCPAY"');
...
test('lines are full-replaced on UPDATE: DELETE then INSERTs', async () => {
  const inv = FIXTURE_ACCPAY[0]!; // AWS, 2 lines
  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, { Invoices: [inv] });
  ...
  const sqlStub = makeSqlStub();
  queueExistingInvoiceRows(sqlStub, 'existing-aws');
  const chainStub = makeChainStub();
...
// -------------------------------------------------------------------------
// 10. Source-external-id matching uses the Xero InvoiceID verbatim.
// -------------------------------------------------------------------------

test('source_external_id is the Xero InvoiceID (forwarded as-is)', async () => {
  const inv = FIXTURE_ACCPAY[1]!; // Officeworks, InvoiceID '22222222-...'
  nock(XERO_API_HOST)
    .get(`${XERO_API_PATH}/Invoices`)
    .query({ where: 'Type=="ACCPAY"', page: '1', pageSize: '100' })
    .reply(200, { Invoices: [inv] });
