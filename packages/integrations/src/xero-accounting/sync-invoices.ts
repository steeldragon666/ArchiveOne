...
export interface SyncInvoicesResult {
   ...
   * undefined to `[]`. The real `syncInvoices` always sets it.
   */
  ...
  while (true) {
    ...
    for (const inv of invoices) {
      ...
      if (existing) {
        ...
        // run would re-insert (UNIQUE on source_external_id prevents a
        // duplicate, but the trigger would dispatch on a stale id).
        // Non-null assertion because we always seed the field at result
        // init (line ~204); the optional `?` on the type is purely so
        // test-stub callers don't have to enumerate it.
