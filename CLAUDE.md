...
## File Organization (cpa-platform-specific)

This is a **pnpm + turbo monorepo**, NOT a flat-/src project. Use the established layout:

- `apps/api/src/` — Fastify API (W0–W3)
- `apps/web/src/` — Next.js consultant portal (W4)
- `packages/auth/src/` — OIDC + JWT + session middleware (W2-W3)
- `packages/db/src/` — Drizzle schemas + Postgres clients (sql, privilegedSql)
- `packages/observability/src/` — pino logger + OTel tracer (W0)
- `packages/schemas/src/` — zod primitives + TenantRef + UserRef (W0, W3)
- `tools/scripts/` — platform-admin CLIs (e.g. onboard-tenant)
- `docs/plans/` — ALL design docs + implementation plans go here (per project)
- `docs/decisions/` — ADRs

**NEVER** use `/src` or `/tests` at the root. Tests live alongside source as `*.test.ts` files.

## Project Architecture (cpa-platform-specific)

- pnpm 10.26.0 + turbo 2 monorepo, Node 22 LTS, TypeScript 5.6 strict
- ESM with `module: NodeNext` — source-side relative imports use `.js` extensions
- Strict TS: `verbatimModuleSyntax`, `isolatedModules`, `exactOptionalPropertyTypes`
- Postgres 16 + Drizzle ORM 0.36 with **real-DB integration tests** (NOT mock-first)
- RLS-enforcing multi-tenant model: cpa_app role + `app.current_tenant_id` GUC
- Two DB clients: `sql` (cpa_app, RLS-enforcing) and `privilegedSql` (cpa, RLS-bypass for auth)
- App-side UUID v4 via `crypto.randomUUID()` (NOT Postgres `gen_random_uuid()`)
- Audit-column convention: `createdAt` / `updatedAt` (notNull, defaultNow, $onUpdate) + `deletedAt` (nullable, soft-delete). Append-only tables (delegation_token) have NO `deletedAt`.
- camelCase TS property names + snake_case SQL column names (Drizzle idiom)
...
# Per-package test
pnpm --filter @cpa/<package> test

# Typecheck + lint + format:check are ALL gates that must pass before commit
